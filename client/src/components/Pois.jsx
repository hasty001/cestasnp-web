  ...
  return (
    <PageWithLoader pageId="Pois" pageTitle={`Dôležité miesta${Constants.WebTitleSuffix}`}>
      <>
        ...
        <button data-nosnippet className="snpBtn pois-map-table-link no-print" title="V tabulke" onClick={() => navigate('/pred/pois/tabulka')}><i className="fas fa-table"></i></button>
        <button data-nosnippet className={"snpBtn pois-map-watch-gps no-print" + (watchGps ? " down" : "")} title="Ukázať moju polohu" onClick={() => toggleWatchGps()}><i className="fas fa-map-marker-alt"></i></button>
        <button data-nosnippet className="snpBtn pois-map-add no-print" onClick={() => navigate('/ucet/pridatpoi' + (gpsMarker ? (`#lat=${gpsMarker.lat}&lon=${gpsMarker.lon}&acc=${gpsMarker.accuracy}`) : ""))}>Pridať</button>
      </>
    </PageWithLoader>);
}
